"""
Plotting functions, should try to convert many of these to plot on axes rather 
than on figures, for convenience on subplots.
...
def plot_background(x, y, yb):
    ...
    ax[0].plot(x, y, lw=1, label="original")
    ax[0].plot(x, bg, lw=2, label="background")
    ax[0].legend()

    ax[1].plot(x, y, lw=1)
    ax[1].plot(x, bg, lw=2)

    ax[1].set_ylim([0.9 * np.min(bg), 1.1 * np.max(bg)])
    ax[2].plot(x, yb, lw=1)
    return fig
