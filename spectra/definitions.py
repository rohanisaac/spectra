...
def find_background(x_data, y_data, percent=3, poly_deg=3, smoothing=5):
    ...
    percent: int (default = 3)
        The percent that data points in the background can be different from
        the previous one
    poly_deg: integer
...
def find_peaks(y_data, lower=1, upper=5, threshold=5, limit=20):
    ...
    peak_pos = signal.find_peaks_cwt(
        y_data, np.arange(lower, upper), min_snr=2)
    ...
    peak_pos = [y for (x, y) in sorted(zip(y_data[peak_pos], peak_pos),
                reverse=True)]
